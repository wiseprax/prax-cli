...
|---|---|---|
| **Praxant** | The platform / project / orchestration system as a whole. Coined from Greek *praxis* (action with practical wisdom) + Latin *-ant* (one who does). Pronunciation: PRAK-sant. | Use when referring to the project, the company-feel umbrella identity, the GitHub org (`praxant-ai`), or the domain (`praxant.ai`). |
...
| **Praxant umbrella** | Praxant as the parent identity for current and future products. Today the only product is Praxagent (the v1 orchestration platform); future products may be added under the same umbrella. | Use when explaining brand structure or planning future products. |
...
| **Handoff report** | A structured artifact (typically a markdown comment on the PR) that a praxagent writes at the end of its phase. Documents what was done, what's still open, what assumptions were made, and what the next phase needs. Inherited from Marins' claude-ai hub design. | Use when discussing inter-agent communication. The handoff report is the canonical "this is what I did" record — both for the next praxagent and for human reviewers. |
...
| **Skill (in the sense Praxant uses)** | Borrowed and adapted from Marins' claude-ai hub design | A markdown file with frontmatter encoding procedural knowledge. NOT the same as "Anthropic Skills" or any specific vendor's skill format. |
| **Hook (in the sense Praxant uses)** | Borrowed and adapted from Marins' claude-ai hub design | A gating/validation mechanism that runs at specific lifecycle points. NOT the same as "Git hooks" or "React hooks." |
