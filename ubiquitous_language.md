...
This document is the canonical vocabulary for WisePrax. It captures the precise meaning of every term used in the project — across documentation, code, commit messages, PR comments, chat conversations with the team, conversations with AI agents (Claude, OpenCode, etc.), grant applications, and marketing copy.
...
| When | What to do |
|---|---|
| Working with an AI agent on WisePrax code or docs | Have this file open in a tab; reference terms by their canonical name |
...
| Term | Definition | Usage notes |
|---|---|---|
| **Agent CLI** | The underlying command-line tool that drives an AI model to do coding work. Examples: `claude`, `codex`, `opencode`, `gemini`, `cursor-agent`. | WisePrax orchestrates agent CLIs by spawning them in containers. Use this term when discussing the underlying tool, not the wrapper. |
| **Agent runtime** | A specific agent CLI configured for use within WisePrax — including its model, harness version, and integration adapter. | Equivalent to "agent CLI" most of the time; use "agent runtime" when emphasizing WisePrax's abstraction over multiple CLIs (SP-4). |
...
WisePrax has six provider abstractions, each with a default v1 adapter and an open extension point.
...
| Term | Definition | Usage notes |
|---|---|---|
| **Provider** | An abstraction over a class of external system (VCS, chat, agent runtime, etc.). Defines the interface WisePrax codes against. | Use "provider" for the interface; use "adapter" for the concrete implementation. |
...
| **Vendor-neutral** | Property of an architecture where every external dependency is behind an abstraction. WisePrax is vendor-neutral across all six provider categories. | Use when describing the strategic positioning. Antonym: vendor-locked. |
| **Sovereign** | Property of a deployment where all data and compute remain under the user's control (their hardware, their VCS, their chat, their model provider, their secrets). No third-party SaaS dependencies. | WisePrax is positioned as sovereign-friendly. Use when describing the audience or value proposition. |
...
| **Subscription auth** | Authentication via a long-lived OAuth token (e.g., `CLAUDE_CODE_OAUTH_TOKEN`, valid 1 year) that bills usage against a personal/team subscription rather than per-token API billing. | Use when discussing how WisePrax connects to AI providers. Subscription auth is core to WisePrax's cost model. |
| **Quota-aware backoff** | The dispatcher behavior of pausing new task dispatches when a provider returns rate-limit responses (HTTP 429). Resumes when the rate-limit window opens. | Reactive, not predictive. Use when discussing how WisePrax respects provider rate limits. |
...
| Term | Definition | Usage notes |
|---|---|---|
| **Developer-workstation threat model** | WisePrax's foundational security stance: protect against realistic threats to a developer's own machine (backups, snapshots, file leaks, screenshots, casual logs) — not against enterprise threats (datacenter compromise, multi-party authorization, regulated audit). See `THREAT_MODEL.md`. | Use when justifying security design choices. The single most important architectural principle for security decisions. |
| **Trust boundary** | The single host where WisePrax runs. Everything inside the host is treated as trusted; everything outside is untrusted. | Use when discussing what WisePrax defends against vs what it doesn't. |
| **Auto-unseal** | OpenBao's behavior of automatically unsealing on host startup using a key stored on the same host. WisePrax's deliberate choice — manual unseal would be friction without realistic benefit at this threat model. | Use when explaining the OpenBao deployment model. Anticipate pushback from enterprise-mindset contributors. |
| **Skeptical empiricism** | WisePrax's design stance: prefer what empirically works today; treat hype with suspicion; require demonstrable results before claiming capabilities. | Use when discussing project values and how to evaluate proposed features or model recommendations. |

---

## 9. Anti-Vocabulary (Terms We Explicitly Avoid)

These terms imply assumptions that contradict WisePrax's design. Use the suggested alternatives instead.

| Avoid | Use instead | Why |
|---|---|---|
| **Sidecar** | Co-located service / separate container in the same Docker network | "Sidecar" is Kubernetes vocabulary that implies pods. WisePrax doesn't use Kubernetes. |
...
| **Operator** (in the K8s sense — a controller process) | Daemon, systemd service | "Operator" in WisePrax = the human user; do not overload. |
...
| **Vibe coding** | (this is what we are NOT — see project-specs.md) | WisePrax is built on the premise that code quality and software fundamentals matter. We are the opposite of vibe-coding. |
| **AI workforce, autonomous engineers, agents that ship features end-to-end** | (be empirical and specific) | Hype language; WisePrax's marketing is honest about what works and what doesn't. |
...
| **Enterprise tier, Pro tier** (for WisePrax features) | (don't pre-build paid tiers without real demand) | Open-core temptation that hurts community trust. WisePrax is OSS first. |
...
These terms come from external sources but are used in WisePrax docs and code. Defined here so usage is consistent.
...
| **Ubiquitous language** | DDD | Shared vocabulary across developers, domain experts, and (for WisePrax) AI agents. This file IS WisePrax's ubiquitous language. |
| **MCP (Model Context Protocol)** | Anthropic | Standard for exposing tools to AI models. WisePrax uses MCP for the human-input tool (and possibly future tools). |
| **OAuth / OAuth Token** | RFC 6749 et al. | The authentication protocol WisePrax uses for subscription-based AI providers. `CLAUDE_CODE_OAUTH_TOKEN` is one example. |
| **Apache 2.0 License** | Apache Software Foundation | WisePrax's chosen license. Permissive, with patent grant + trademark protection. |
| **DCO (Developer Certificate of Origin)** | Linux Foundation | WisePrax's contributor agreement model. Each commit must have `Signed-off-by:` trailer. Lighter than CLA, preserves relicensing optionality. |
| **TDD (Test-Driven Development)** | Kent Beck et al. | The practice of writing tests before implementation. Encouraged for WisePrax code; required by phase-boundary skills (`python-testing-phase`). |
| **Skill (in the sense WisePrax uses)** | WisePrax | A markdown file with frontmatter encoding procedural knowledge. NOT the same as "Anthropic Skills" or any specific vendor's skill format. |
| **Hook (in the sense WisePrax uses)** | WisePrax | A gating/validation mechanism that runs at specific lifecycle points. NOT the same as "Git hooks" or "React hooks." |
...
*This file is the source of truth for what every WisePrax term means. When in doubt, this file wins. When this file is wrong, fix it here first, then fix everywhere else.*
