...
- **Praxant AI** — the umbrella organization behind the project. Use when referring to the team, the entity, governance, brand, or external positioning ("the Praxant AI team", "Praxant AI is the umbrella for…").
...
A raw `claude` session is not a praxagent. A praxagent is the runtime *plus the wrapper*. `praxant.ai` is the canonical domain — not an identity term.
...
| `references/` | Directory of studied prior art. One file per project (or category), plus `COMPARISON.md` — the single matrix + "why Praxant is special" doc. Start with `references/README.md`. |
