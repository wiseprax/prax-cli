...
Praxant runs agents on your machine, under your control, while letting you use the commercial AI runtimes and providers that already work best for your team: Claude Code, Codex, Kimi, Qwen, DeepSeek, and others.
...
Praxant is a wrapper around those tools, not a clone of them and not an unofficial hosted proxy. The goal is to orchestrate official commercial agent runtimes in a developer-controlled workflow.
...
Praxant is built for the middle path:
...
- Wrap official tools instead of replacing them. Praxant is designed to sit around commercial agent CLIs and provider integrations, so teams can keep using the tools they already trust.
- Provider-neutral by design. Praxant is built around adapters, so your workflow does not need to be rewritten every time the model landscape shifts.
- Runtime and model configuration stay flexible. Providers, models, and execution behavior can be selected through Praxant configuration and CLI-level orchestration instead of being hardwired into one vendor workflow.
...
- Better than one-off scripts. Praxant treats agent work as an operational workflow: dispatch, supervise, review, and merge.
...
Praxant is a platform for orchestrating autonomous coding agents.

In Praxant terminology:

- `Praxant` is the platform
...
- Praxant can use supported commercial tools directly instead of pretending to be them
...
Praxant complements strong commercial AI systems. It gives them structure, control, and team workflow.

It is designed to work with commercial tools, not to circumvent them. Provider terms and policies still apply, but Praxant's architecture is intentionally centered on wrapping official runtimes rather than proxying them through an opaque third-party service.
...
Praxant is not trying to prove that agents can type code in a terminal.
...
Praxant is the stable layer above that churn.
...
- Configurable provider, runtime, and model selection through Praxant parameters and orchestration
...
Praxant is in early design and architecture definition.
...
`Praxant` comes from `praxis` plus the agentive `-ant`:

- `praxis`: action, doing, practical work
- `-ant`: one who acts

The name fits the system because Praxant is about agents that act inside real engineering workflows, not just assist in chat.
